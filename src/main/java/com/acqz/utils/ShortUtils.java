...
public class ShortUtils {
    ...
    /**
     * 一个长链接 URL 转换为 4 个短 KEY
     　思路：
     　　1)将长网址 md5 生成 32 位签名串,分为 4 段, 每段 8 个字节;
     　　2)对这四段循环处理, 取 8个字节, 将他看成 16进制串与 0x3fffffff(30位1)与操作, 即超过30位的忽略处理;
     　　3)这 30位分成 6段, 每 5位的数字作为字母表的索引取得特定字符, 依次进行获得6位字符串;
     　　4)总的 md5 串可以获得 4个 6 位串; 取里面的任意一个就可作为这个长 url 的短 url 地址;
